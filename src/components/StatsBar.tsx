  ...
  return (
    <div className="flex-1 flex flex-col gap-1">
      <div className="flex items-center justify-between">
        ...
        <span className={`text-xs font-bold tabular-nums ${warning ? 'text-alert-red stat-glow' : color}`}>
          {displayValue}
        </span>
      </div>
...
export function StatsBar({ sanity, battery, cash }: StatsBarProps) {
  ...
  return (
    <div className="flex flex-col gap-1 py-2 px-4 bg-noir-surface/70 border-b border-noir-border/60 backdrop-blur-sm">
      {showWarning && (
